...
/// The channel automatically registered by this package on native platforms.
const MethodChannel _channel = MethodChannel('dev.milanko.dart_pdf_printing');

// Each native channel owns one accumulated job. Reject overlapping calls so
// a second beginJob cannot silently replace the first document's pages.
...
Future<void> printDocumentPages(
  ...
  if (!_activeChannels.add(channel.name)) {
    throw PlatformException(
        code: 'print_in_progress',
        message: 'A print job is already active on this channel.');
  }
  try {
    await _printDocumentPages(pdfBytes,
        name: name,
        channel: channel,
    ...
    _activeChannels.remove(channel.name);
  }
}

Future<void> _printDocumentPages(
  Uint8List pdfBytes, {
  required String name,
  required MethodChannel channel,
  void Function(int rendered, int total)? onProgress,
  required bool useDocumentPageSize,
  PrintDestination? destination,
}) async {
