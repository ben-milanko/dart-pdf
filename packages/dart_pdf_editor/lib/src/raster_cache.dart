...
class PdfRasterCache {
  ...
  String _thumbKey(int pageIndex, int pixelWidth) =>
      '$documentKey/t$pixelWidth/$pageIndex';
  ...
  Future<ui.Image?> loadThumbnail(int pageIndex, int pixelWidth) async {
  ...
  Future<void> storeThumbnail(
      int pageIndex, int pixelWidth, ui.Image image) async {
    if (documentKey.isEmpty) return;
    try {
      final data = await image.toByteData(format: ui.ImageByteFormat.png);
      if (data == null) return;
      await cache.write(
          _thumbKey(pageIndex, pixelWidth), data.buffer.asUint8List());
    } catch (_) {
