...
// generator's raw SoA arrays (built in pdf_graphics' raster core, or by
// hand in tests), builds the vertex buffer + alpha atlas, and issues one
// draw. Wiring GpuPdfDevice's fill routing through it comes with the shared
// strip generator.
//
// SoA layout contract (must match pdf_graphics/lib/src/raster/
// strip_generator.dart):
//  - xy:          Int32List, per strip `x | (y << 16)` - unsigned 16-bit
...
//                 strip's first column in [alphas]; [stripSolidOffset]
...
/// Sentinel [StripBatch.setStrips] alphaOffset marking a solid strip:
/// coverage is 1 across the whole quad and no alpha-atlas data is consumed.
const int stripSolidOffset = 0xFFFFFFFF;
...
class StripBatch {
  ...
  void setStrips({
    ...
    for (var i = 0; i < count; i++) {
      ...
      final solid = off == stripSolidOffset;
      ...
      // 6 corners as (x, y, texel, yflags):
      void corner(double px, double py, double texel, double yFlags) {
        v[o] = px;
        v[o + 1] = py;
        v[o + 2] = texel;
        v[o + 3] = yFlags;
      ...
      corner(x0, y0, t0, yTop);
      corner(x1, y0, t1, yTop);
      corner(x1, y1, t1, yBot);
      corner(x0, y0, t0, yTop);
      corner(x1, y1, t1, yBot);
      corner(x0, y1, t0, yBot);
    }
