#!/usr/bin/env bash
# Release Chrome A/B for #242. Keeps the local CAD corpus out of git.
set -euo pipefail
cd "$(dirname "$0")/.."

ROOT=../../..
PDF="$ROOT/corpus/ly9-far-cad.pdf"
PORT=${CAD_PERF_PORT:-8765}
...
class Handler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/__cad_web_cold_start_benchmark__':
            result = urllib.parse.parse_qs(parsed.query).get('result', [''])[0]
            print('\nCAD_WEB_COLD_START_BENCHMARK_' + result, flush=True)
...
fvm flutter run -d chrome --release -t lib/web_cold_start_benchmark.dart \
  ...
  --dart-define="CAD_PERF_MIN_VISIBLE_SPEEDUP=${CAD_PERF_MIN_VISIBLE_SPEEDUP:-1.10}" \
  --dart-define="CAD_PERF_MAX_WARM_REGRESSION=${CAD_PERF_MAX_WARM_REGRESSION:-1.10}"
