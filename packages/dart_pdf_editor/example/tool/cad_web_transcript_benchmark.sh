#!/usr/bin/env bash
# Release Chrome A/B for #241. Keeps the local CAD corpus out of git.
set -euo pipefail
cd "$(dirname "$0")/.."
...
class Handler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/__cad_web_transcript_benchmark__':
            result = urllib.parse.parse_qs(parsed.query).get('result', [''])[0]
            print('\nCAD_WEB_TRANSCRIPT_BENCHMARK_' + result, flush=True)
...
fvm flutter run -d chrome --release -t lib/web_transcript_benchmark.dart \
  ...
  --dart-define="CAD_PERF_MIN_SPEEDUP=${CAD_PERF_MIN_SPEEDUP:-1.10}"
