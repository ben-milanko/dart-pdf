...
Batch 5, session 3 (threaded comments + transport-agnostic sync). Two
layers on top of the batch-5 annotation-sync primitives (/NM identity,
PdfAnnotationSnapshot, pdfDiffAnnotations, upsert/removeByName,
applyRemoteChange).

PART A — reply threads (§12.5.6.x). Reading model on `PdfAnnotation`
(annotation.dart): `inReplyTo` (resolves /IRT → the parent's /NM — IRT is
an *indirect ref* to the parent dict, surfaced as the durable name),
`replyType` (/RT, R/Group), `reviewState`/`stateModel` (/State,
/StateModel), `subject` (/Subj), `creationDate`/`modificationDate`
(/CreationDate, /M via a shared lenient `_parsePdfDate`; `pdfFormatDate`
writes the UTC `D:…Z00'00'` form), and the two predicates the renderer and
sidebar key on: `isReply` (has /IRT and /RT R-or-absent) and
`isStateAnnotation` (has /StateModel). Thread model (comments.dart, new
export): `PdfReviewState` enum (Review model Accepted/Rejected/Cancelled/
Completed/None + Marked model Marked/Unmarked, each carrying its
/StateModel), `PdfComment` (annotation + nested `replies` + `stateHistory`),
`PdfCommentThread` (root + `comments` pre-order + `replyCount` + `state` =
newest state entry across the thread + `isResolved` = state Completed).
`PdfCommentThread.forPage` links replies by **dict identity** (resolve /IRT
to a CosDictionary and match — doesn't need /NM present) and attaches state
annotations to the comment they reply to; `of()` finds a member's thread.
Authoring (comment_editor.dart, new part of editor.dart):
`PdfCommentEditing.replyToAnnotation` (a /Text markup with /IRT→target,
/RT R, /Contents, /T, /CreationDate, /M), `setReviewState` (a state reply:
empty /Contents + /State + /StateModel per §12.5.6.2 — set a NEW one to
change status so the history survives), `resolveThread`/`reopenThread`
(Completed/None). Replies and state replies carry **no /AP**:
`_addThreadAnnotation` is `_addAnnotation` minus the appearance (the
/Annots linking was factored into `_linkAnnotation`). The interpreter
(pdf_graphics drawAnnotations) now SKIPS isReply/isStateAnnotation
alongside Popups — thread content shows in the comment pane, not as a
second on-page icon (matches Acrobat; verified the Ghent/pdfjs render
baselines don't move, and the checked-in corpora carry no genuine
/IRT/StateModel — the few `IRT` byte hits are coincidental stream data).

Threads sync through the SAME snapshot/diff machinery: /IRT can't travel
detached, so `PdfAnnotationSnapshot` gained `inReplyTo` (the parent's /NM,
captured only with keepName; /RT is also kept then), carried in toJson as
`irt`, and `pasteAnnotation` RELINKS it — finds the parent by name in the
receiving document (`_findByName`, shared across the editor.dart parts) and
sets /IRT to a fresh indirect ref (orphaned cleanly when the parent is
absent, so a stray reply stays valid rather than dangling). Controller
(editing_controller.dart): `replyToAnnotation`/`setReviewState`/
