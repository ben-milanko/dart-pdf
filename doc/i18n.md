...
**All of tier 1 ships** — Spanish landed first
(`doc/dev-log/2026-07-23-i18n-spanish-locale.md`), then the rest of the tier
(`doc/dev-log/2026-07-23-i18n-tier1-locales.md`): `supportedLocales` is now
`[ar, de, en, es, fr, hi, ja, pt, ru, zh]`, and Arabic makes it the first
shipped RTL locale. Brazilian Portuguese and Simplified Chinese ship under
their **base** tags `pt`/`zh` (not `pt_BR`/`zh_Hans`): gen-l10n requires a base
fallback for any script/region-coded locale, and Flutter's framework
localizations already treat bare `pt` as Brazilian and bare `zh` as Simplified
(the explicit variants are `pt_PT`/`zh_Hant`). Two pieces landed with Spanish:
...
Still open: the tier-2 locales (`zh_Hant`, ko, it, tr, vi, id, pl, nl, th, uk —
each just drops a `<prefix>_<locale>.arb` next to the template and regenerates,
and the picker and coverage gate pick it up automatically); engine-error→UI-
message mapping; and `DateFormat`/`NumberFormat` for month abbreviations and
numeric readouts.
