...
restore_worker_placeholder() {
  git update-index --no-assume-unchanged "$WORKER_ASSET" 2>/dev/null || true
...
# Pub's package validator rejects a checked-in file whose working-tree content
# differs from HEAD. This one difference is deliberate: the repository keeps a
# small placeholder, while the published package must contain the generated
# worker. Hide only this exact path from git status while validation/publishing
# reads the generated bytes from disk; the EXIT trap restores the index flag and
# placeholder even when a publish command fails.
git update-index --assume-unchanged "$WORKER_ASSET"
