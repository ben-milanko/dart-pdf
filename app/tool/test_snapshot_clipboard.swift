...
struct ClipboardTest {
  static func main() {
    ...
    clipboard.markLocalCopy()
    precondition(clipboard.readExternalPdf() == nil, "local Copy supersedes an unpasted external PDF")
    precondition(pasteboard.data(forType: .pdf) == pdf, "marking must not overwrite clipboard data")
    pasteboard.clearContents()
    pasteboard.setData(pdf, forType: .pdf)
    precondition(clipboard.readExternalPdf() == pdf)
    pasteboard.clearContents()
